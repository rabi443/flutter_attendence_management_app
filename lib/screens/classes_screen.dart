...
class _ClassesScreenState extends State<ClassesScreen> {
  ...
  List<String> get tableHeaders {
    if (widget.endpoint == 'classes') {
      return ["ID", "Class Level", "Action"];
    }
  ...
  void deleteItem(dynamic id) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          success == true ? "Deleted successfully" : "Failed to delete",
        ),
      ),
    );
  ...
  void openForm({Map<String, dynamic>? item}) {
    ...
    // Only relevant fields for classes
    List<String> allowedFields = widget.endpoint == 'classes'
        ? ['class_name']
        : ['name', 'email', 'role']; // example for other endpoints
